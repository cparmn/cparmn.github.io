...
Adding a disk to an exsting LVM takes a two step appach as you first need to gather information about the current configuration.
The process descripbed below takes place after the disk has been added to the Host Machine.
...
You'll also need to understand the information of the LVM.  This can be acomplihsed by looking at three different parts of LVM.  
...
2. Volume Groups (VG)
  ...
  To view Volume Group informaiton use `vgs` or `vgdisplay`
...
In order to add a disk to an LVM you need to Initialize a physical volume to be used by LVM.  This is accomplished with the `pvcreate` command as shown bewow.  The disk will be the same disk you verified with the `fdisk` command in this case, `/dev/mapper/mpathf` due to the fact that I'm using multipathing 
...
You'll notice that the new disk shows up at the bottom of the list.  Now you can add the Physcial Volume to a Volume group, this is done with the `vgextend` command. In this case I'll be added the Physical volume to the HotStorage Volume Group.
...
```
 ...
 ```
 As you can see we now have 5.0 T of Free Space, but this is currently only assoicated with the Volume Group.  If you view the Logical Volume this is currently unchanged as shown below.
...
Here it is important to understand the fileystem.  if you're using ext2, ext3 or ext4 you'll be able to use the `resize2fs` command, however in this case the file system is an xfs.  In order to verify the filesystem you can use the `df` command with the `-T` argument as shown below.
...
Note: using `xfs_growfs` withough any addtional agurments the filesystem will be increased to the maximum available size. in order to sepcify a specific size use `-D SIZE`
